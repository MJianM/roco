...
class DialogPrompter:
    ...
    def __init__(
        ...
        assert llm_source in ["gpt-4", "gpt-3.5-turbo", "claude", "meta-llama/Meta-Llama-3.1-8B-Instruct"], f"llm_source must be one of [gpt4, gpt-3.5-turbo, claude], got {llm_source}"
    ...
    def query_once(self, system_prompt, user_prompt, max_query):
        ...
        for n in range(max_query):
            ...
            try:
                if model_type == 'gpt':
                    client = AzureOpenAI(
                        azure_endpoint="https://tzydemo1219.openai.azure.com/",
                        api_key="732a9a231689443eb067de5d345550fa",
                        api_version="2024-02-01"
                    )
                    response = client.chat.completions.create(
                        model="gpt-4o", #model="TzyGPT41106",#model="meta/llama-3.1-405b-instruct",
                        messages=[
                            # {"role": "user", "content": ""},
                ...
                time.sleep(20 if model_type == 'llama' else 60)
            continue
        # breakpoint()
